    ...
    <header id="top" onPointerMove={onMove} onPointerLeave={() => setTilt({ x: 0, y: 0 })} className="relative flex min-h-svh items-center overflow-hidden px-[4vw] pb-16 pt-32">
      ...
      <div className="relative z-10 mx-auto grid w-[min(1200px,92vw)] items-center gap-12 lg:grid-cols-[1.05fr_.95fr]">
        <div className="hero-enter">
          ...
          <p className="max-w-[44ch] text-[clamp(1rem,1.4vw,1.1rem)] leading-8 text-ink/60">Six botanical soaps, poured, cut and cured by hand in our Pune workshop. Pure ingredients, pure care — pressed into every 45-gram bar.</p>
          ...
          <ul className="flex list-none flex-wrap gap-x-6 gap-y-3 text-[.68rem] font-bold uppercase tracking-[.16em] text-ink/60">
            {["6 botanical blends", "45 g per bar", "4-week cure", "Zero synthetics"].map((item, index) => <li key={item} className={index ? "border-l border-ink/15 pl-6" : ""}>{item}</li>)}
          </ul>
        </div>
        <div className="relative mx-auto flex w-full max-w-[560px] flex-col items-center [perspective:1100px]">
          ...
          <div className="w-[min(27rem,92%)] transition-transform duration-300 ease-out [transform-style:preserve-3d]" style={{ transform: `rotateX(${tilt.x}deg) rotateY(${tilt.y}deg)` }}><div className="soap-float"><SoapBar product={saffron} /></div></div>
          ...
          <p className="mt-6 flex items-center gap-4 text-[.66rem] font-bold uppercase tracking-[.2em] text-ink/40 before:h-px before:w-9 before:bg-ink/15">№ 004 · Royal Saffron — the signature pour</p>
        </div>
      </div>
    </header>
  );
