...
export function CartDrawer({ open, cart, onClose, onChange, onCheckout }: { open: boolean; cart: Record<string, number>; onClose: () => void; onChange: (id: string, amount: number) => void; onCheckout: () => void }) {
    ...
    <div className={`fixed inset-0 z-[70] transition ${open ? "visible" : "invisible delay-500"}`}>
      ...
      <aside role="dialog" aria-modal="true" aria-label="Shopping bag" className={`absolute bottom-0 right-0 top-0 flex w-full max-w-[440px] flex-col bg-paper shadow-2xl transition-transform duration-500 ease-out ${open ? "translate-x-0" : "translate-x-full"}`}>
        ...
        <div className="flex-1 overflow-y-auto px-6">
          {!items.length ? <div className="grid h-full place-items-center text-center"><div><ShoppingBag className="mx-auto h-8 w-8 text-gold" strokeWidth={1.3} /><p className="display mt-5 text-3xl">Every petal awaits.</p><button onClick={onClose} className="mt-3 text-xs font-bold uppercase tracking-[.16em] underline">Browse the collection</button></div></div> : items.map((product) => <div key={product.id} className="flex gap-4 border-b border-ink/15 py-5"><div className="grid h-20 w-24 place-items-center overflow-hidden"><SoapBar product={product} /></div><div className="flex-1"><h3 className="display text-2xl">{product.name}</h3><p className="mt-1 text-xs text-ink/50">₹{product.price} · 45 g</p><div className="mt-3 flex w-fit items-center rounded-full border border-ink/15"><button onClick={() => onChange(product.id, -1)} className="p-2" aria-label={`Decrease ${product.name}`}><Minus className="h-3 w-3" /></button><span className="min-w-7 text-center text-xs font-bold">{cart[product.id]}</span><button onClick={() => onChange(product.id, 1)} className="p-2" aria-label={`Increase ${product.name}`}><Plus className="h-3 w-3" /></button></div></div><span className="text-sm font-bold">₹{product.price * cart[product.id]}</span></div>)}
        </div>
